...
export async function normalizeImage(acquired: AcquiredImage, maxDecodedPixels: number): Promise<NormalizedImage> {
  ...
  try {
    ...
    throw new VisionError('DECODE_FAILED', 'The image metadata could not be decoded.', {
  ...
  if (!width || !height || !Number.isSafeInteger(width) || !Number.isSafeInteger(height)) {
    throw new VisionError('DECODE_FAILED', 'The decoded image dimensions are missing or invalid.', {
      details: { stage: 'image_decode' },
    })
  }
  const pixelCount = width * height
  if (!Number.isSafeInteger(pixelCount) || pixelCount > maxDecodedPixels) {
    throw new VisionError('PIXEL_LIMIT_EXCEEDED', 'The decoded image exceeds the configured pixel limit.', {
      details: { stage: 'image_decode', maxPixels: maxDecodedPixels },
    })
  }
  ...
  try {
    ...
    if (info.channels !== 3) {
      throw new VisionError('DECODE_FAILED', 'The image could not be normalized to three-channel RGB.', {
        details: { stage: 'image_decode' },
      })
    }
    return {
    ...
    throw new VisionError('DECODE_FAILED', 'The image pixels could not be decoded.', {
      details: { stage: 'image_decode' },
      cause: error,
    })
  }
